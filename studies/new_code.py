#we try time trace cooling with np. arrays now!!!!!!!!!!!
...
def respL( Dt ):
   #if np.any( (Dt < 1.*wlL) | (Dt > 1.*wrL) ): 
    #  for Dt_ in Dt[(Dt< 1.*wlL | Dt > 1.*wrL)]:
     #   print( f' ===> function respL called with Dt ={1e6*Dt_:8.4f}' )
...
#   first generate transverse trace
    ...
    for x, xp, delt, Tau in ptsMain:     #slice under investigation
        start = time.time() 
        ind = np.arange( max(-NSampext, math.ceil((wlH+Tau+Th/2)/DTSamp)), 
    ...
    for x, xp, delt, Tau in ptsMain:
        ind = np.arange( max(-NSampext, math.ceil((wlL+Tau+Th/2)/DTSamp)), 
    ...
    return TraceH.tolist(), TraceL.tolist()
...
def TimeTraces( ptsEarly, ptsMain, ptsLate ):
    ...
    for x, xp, delt, Tau in ptsMain:    #precious slice than the one under investigation
        ind = np.arange(max(-NSampext, math.ceil((wlH+Tau+Th/2)/DTSamp)),  
                        min(NSamp+1+NSampext, math.ceil((wrH+Tau+Th/2)/DTSamp)) )
        TraceH[ind] += x*respH(ind*DTSamp - Th/2 - (Tau) )    
        
    for x, xp, delt, Tau in ptsLate:    #precious slice than the one under investigation
        ind = np.arange(max(-NSampext, math.ceil((wlH+Tau+Th+Th/2)/DTSamp)),  
                        min(NSamp+1+NSampext, math.ceil((wrH+Tau+Th+Th/2)/DTSamp)) )
        TraceH[ind] += x*respH(ind*DTSamp - Th/2 - (Tau+Th))  
...
def compute_cubic_spline(DTSamp, NSampext, NSamp, Th, TrHBlue):
    ...
    #y = TrHBlue[-NSampext:]+TrHBlue[:NSamp + 1 + NSampext]
