...
from kick import TimeTraces,kickH, kickL
from responses import respH,respL 



def Cooling(Nparts=2000, Nturns=10000, gain=0.0025, gainL=4e-8, sigdelt=0.0001,NSamp=800, random_seed=2207245,over_sample_factor=10 ):
...
# Plot Geometric Emittance (Red Curve)
    ax4.plot(pa / total_angle, rms_momentum_spread[2:]*1000, label='dp/p', color='red')

# Add labels and title
    ax4.set_xlabel('Turns (Normalized)', fontsize=16)
    ax4.set_ylabel('RMS momentum_spread(milli)', fontsize=16)
    ax2.set_title('RMS momentum spread Over Turns', fontsize=16)
    ax4.grid(True)
# Add a legend
